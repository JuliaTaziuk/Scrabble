...
def get_word_score(word, n):
    ...
    res = first_component * second_component
    return res
...
def is_valid_word(word, hand, word_list):
    ...
    """
    word_lower = word.lower()
    word_lst = get_frequency_dict(word_lower)
    wildcard = '*'
    wcard_count = word_lower.find(wildcard)
    
    if word_lower in word_list:
            for letter in word_lst.keys():
                if word_lst[letter] > hand.get(letter, 0):
                    return False
            return True
    else:
        if wildcard in word:
            for vowel in VOWELS:
                word_2 = word_lower[:wcard_count] + vowel + word_lower[wcard_count + 1:]
                if word_2 in word_list:
                    return True
    ...
    while len(hand.keys()) > 0:
        ...
        # Otherwise (the input is not two exclamation points):
            ...
            if is_valid_word(word, hand, word_list):
                ...
                print (f'"{word}" earned {word_points} points.')
    ...
    for i in range(hands_number):
        ...
        if replay_hand.lower() == 'no':
            ...
            if substitution:
                ...
                if answer.lower() == 'yes':
                    letter = input("Which letter would you like to replace: ")
                    new_hand = substitute_hand(new_hand, letter.lower())
                    substitution = False

            hand_score = play_hand(new_hand, word_list)

        all_total_sc += hand_score
    print(f'Total score over all hands: {all_total_sc}')
